...
bool isBalanced(const BinaryTreeNode* treeRoot)
{
    ...
    // nodes will store pairs of a node and its depth
    ...
    while(!nodes.empty()){
        ...
        //case: hit a leaf node!
        if(!node->left_ && !node->right_){
            
            //only concerned with new depths
            if(depthCount == 0 || find(depths, depths+depthCount, depth) == depths+depthCount){
                depths[depthCount] = depth;
                depthCount++;
            }
            
            if(depthCount > 2 || (depthCount == 2 && (max(depths[0], depths[1]) - min(depths[0], depths[1]) > 1))){
                return false;
            }
